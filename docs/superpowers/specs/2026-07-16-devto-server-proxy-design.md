...
- `/api/devto/articles/:id` calls `https://dev.to/api/articles/:id` without authentication because article details are public.
...
The article ID handler validates that the route parameter is present before making its upstream request.
...
- Invalid or missing article ID: return HTTP 400.
...
- Missing key and invalid ID paths return sanitized errors.
